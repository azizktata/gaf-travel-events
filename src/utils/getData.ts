...
const HOTEL_QUERY_SLUG = defineQuery(
...
const VOYAGE_QUERY_SLUG = defineQuery(
...
export async function fetchHotelsByFilterOpions(
  typeFilter: string,
  destinationFilter: string
) {
  const { data } = await sanityFetch({
    query: HOTEL_FITLER_QUERY(typeFilter, destinationFilter),
  });
  return data;
}
...
export async function fetchVoaygesByFilterOpions(
  typeFilter: string,
  destinationFilter: string
) {
  const { data } = await sanityFetch({
    query: VOYAGE_FILTER_QUERY(typeFilter, destinationFilter),
    revalidate: 0,
  });
